...
import { Plus, Ruler, Edit2, Power, PowerOff } from 'lucide-react';
...
type TypeOption = SingleValue<{ value: MeasurementTypeOption; label: string }>;

const MEASUREMENT_TYPE_LABELS: Record<MeasurementTypeOption, string> = {
    MASS: 'Masa',
    VOLUME: 'Volumen',
    UNIT: 'Unidad',
    PACKAGE: 'Paquete / Empaque'
};

const MEASUREMENT_TYPE_OPTIONS = (Object.keys(MEASUREMENT_TYPE_LABELS) as MeasurementTypeOption[]).map((value) => ({
    value,
    label: MEASUREMENT_TYPE_LABELS[value]
}));
...
export default function UnitsOfMeasurePage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const systemFactor = Number(formData.systemFactor);
        ...
        if (!Number.isFinite(systemFactor) || systemFactor <= 0) {
            showError('El factor de referencia debe ser mayor a 0');
            return;
        }
                    ...
                    <div key={unit.id} className={`unit-card entity-card-new ${unit.active ? '' : 'inactive'}`}>
                        ...
                        <div className="unit-card-body entity-card-body">
                            ...
                            <div className="unit-meta">
                                <span>Factor ref.: {Number(unit.systemFactor).toLocaleString()}</span>
                            </div>
                            <p className="unit-meta-hint">
                                {unit.measurementType === 'MASS' && 'Referencia en gramos (ej. kg = 1000)'}
                                {unit.measurementType === 'VOLUME' && 'Referencia en mililitros (ej. l = 1000)'}
                                {unit.measurementType === 'UNIT' && 'Conteo por unidad'}
                                {unit.measurementType === 'PACKAGE' && 'Empaque; el peso real se define por producto'}
                            </p>
                        </div>
                        {canMutate && (
                    ...
                    <div className="modal-tab-content">
                        <div className="modal-input-group">
                            ...
                            <input
                                ...
                                placeholder="Ej: Gramo, Quintal, Paquete Mozzarella 3.5g"
                                required
                            />
                        </div>
                        <div className="modal-input-group">
                            <label className="modal-input-label">Abreviatura (única en la empresa)</label>
                            <input
                                type="text"
                                className="modal-standard-input"
                                value={formData.abbreviation}
                                onChange={(e) => setFormData((prev) => ({ ...prev, abbreviation: e.target.value }))}
                                placeholder="Ej: g, qq, pkg_moz_3_5g"
                                required
                            />
                        </div>
                        <Select
                            ...
                            value={MEASUREMENT_TYPE_OPTIONS.find((o) => o.value === formData.measurementType) || null}
                            onChange={(option: TypeOption) =>
                                option && setFormData((prev) => ({ ...prev, measurementType: option.value }))}
                            isSearchable={false}
                        />
                        <div className="modal-input-group">
                            <label className="modal-input-label">Factor de referencia del sistema</label>
                            <input
                                type="number"
                                min="0.000001"
                                step="0.000001"
                            ...
                            <small className="modal-input-hint">
                                Masa: gramos (g=1, kg=1000). Volumen: ml (ml=1, l=1000). Paquetes personalizados: use 1 aquí y defina el peso en cada producto.
                            </small>
                        </div>
                        {editingUnit && (
