    ...
    return (
        <div className={`menu-item-card ${item.category.name.toLowerCase().replace(/\s+/g, '-')}`}>
            ...
            <div className="menu-card-actions">
                <button className="menu-action-btn view" onClick={(e) => { e.stopPropagation(); onClick(item); }}>
                    <Eye size={20} />
                    <span>Ver</span>
                </button>
                <button className="menu-action-btn edit" onClick={(e) => { e.stopPropagation(); onEdit(item); }}>
                    <Edit2 size={20} />
                    <span>Editar</span>
                </button>
                <button className="menu-action-btn delete" onClick={(e) => { e.stopPropagation(); onDelete(item); }}>
                    <Trash2 size={20} />
                </button>
            </div>
        </div>
