          ...
          <tbody>
            {concepts.map((concept, index) => (
              <tr key={`${concept.code}-${index}`}>
                ...
                <td><select value={concept.type} onChange={(event) => changeType(index, event.target.value as HrPayrollPaymentConceptDefinition['type'])}><option value="INCOME">Ingreso</option><option value="DEDUCTION">Deducción</option></select></td>
                ...
                <td>{concept.type === 'INCOME' ? <select aria-label={`Tratamiento IR ${concept.code || index + 1}`} value={concept.incomeTaxTreatment ?? 'NONE'} onChange={(event) => update(index, { incomeTaxTreatment: event.target.value === 'NONE' ? null : event.target.value as HrPayrollPaymentConceptDefinition['incomeTaxTreatment'] })}><option value="NONE">No sujeto</option><option value="REGULAR_FIXED">Ordinario fijo</option><option value="REGULAR_VARIABLE">Ordinario variable</option><option value="OCCASIONAL">Ocasional</option></select> : <label><input type="checkbox" checked={concept.incomeTaxDeductible} onChange={(event) => update(index, { incomeTaxDeductible: event.target.checked })} /> Deducción IR autorizada</label>}</td>
