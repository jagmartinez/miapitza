...
function runFixture() {
    return {
        ...
        netPay: new Prisma.Decimal('900.00'), employeeCount: 1,
        ...
        anomalies: [], coverageClaims: [{ userId: 11, release: null }], receipts: [],
    };
}
...
describe('payroll parallel reconciliation', () => {
    ...
    function arrange(run = runFixture()) {
        ...
        const tx = {
            payrollRun: { findFirst: jest.fn().mockResolvedValue({
                configurationRevision: { review: { decision: 'VALIDATED' } }, attendanceDependencies: [],
                snapshots: [{ userId: 11, summaryRevisions: [], aguinaldoIncomeSegments: [] }], kind: 'REGULAR', netPay: new Prisma.Decimal('900.00'),
            } as never) },
            payrollCoverageClaim: { count: jest.fn().mockResolvedValue(1 as never) },
            payrollComponent: { findFirst: jest.fn().mockResolvedValue(null as never) },
        };
    ...
    it('returns a reproducible all-green report without asserting legal or production certification', async () => {
        ...
        const result = await PayrollRunService.reconcileParallelControl(4, 3, 40, 'REGULAR', {
            expectedGrossIncome: '1000.00', expectedTotalDeductions: '100.00', expectedNetPay: '900.00',
    ...
    it('reports external differences explicitly instead of accepting a tolerance or fallback', async () => {
        arrange();
        const result = await PayrollRunService.reconcileParallelControl(4, 3, 40, 'REGULAR', {
            expectedGrossIncome: '999.99', expectedTotalDeductions: '100.00', expectedNetPay: '899.99',
            expectedEmployeeCount: 1, controlSource: 'Cálculo paralelo revisado', evidenceReference: 'evidence://pay-01',
        });
